...
- **Compatibilidade local**
  - `npm run dev` usa `vite` com proxy local de `/api/*`.
  - a origem desse proxy é configurável via `VITE_LOCAL_API_ORIGIN` e aponta por defeito para `https://mediadex.app`.
  - produção e preview oficiais usam exclusivamente `functions/api/*` em Cloudflare Pages.
  - novas alterações backend devem entrar em `functions/api/*`; não existe runtime Netlify suportado no projeto.
