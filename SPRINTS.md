...
Estado atual: concluído até **S3-T04**.
...
- [ ] S3-T05 Ajustar import/export para suportar os 3 tipos.

### Critérios de aceitação
- [ ] Toda a funcionalidade de séries continua funcional.
- [ ] Estrutura já suporta filmes e livros sem hacks.
- [x] Migração local preserva dados existentes de séries.

## Sprint 4: Integração Filmes e Livros (APIs grátis)

### Tarefas
- [ ] S4-T01 Filmes: usar TMDb (`/movie`) com chave já existente.
- [ ] S4-T02 Livros: integrar Google Books API (com key).
- [ ] S4-T03 Livros fallback: Open Library para robustez de resultados/capas.
- [ ] S4-T04 Criar pesquisa, detalhe e adicionar à biblioteca para filmes.
- [ ] S4-T05 Criar pesquisa, detalhe e adicionar à biblioteca para livros.
- [ ] S4-T06 Regras de progresso: filme visto/não visto; livro páginas ou % lido.

### Critérios de aceitação
- [ ] É possível adicionar/remover/arquivar filme e livro.
- [ ] Detalhes de filme/livro carregam com fallback funcional.
- [ ] Limites/erros de API têm mensagem clara e retry.

## Sprint 5: Dashboard Nova (3 quadros grandes) + Navegação

### Tarefas
- [ ] S5-T01 Criar dashboard inicial com 3 cards grandes: Séries, Filmes, Livros.
- [ ] S5-T02 Cada card mostra métricas rápidas (total, em progresso, concluídos).
- [ ] S5-T03 Navegação por secção com botão claro “Voltar à Dashboard”.
- [ ] S5-T04 Manter identidade visual atual (cores, fonts, spacing, tokens).
- [ ] S5-T05 Responsividade e acessibilidade (teclado, foco, labels ARIA).

### Critérios de aceitação
- [ ] Dashboard é ecrã inicial padrão.
- [ ] Navegação entre dashboard e secções é fluida e previsível.
- [ ] UI de filmes/livros mantém o mesmo look & feel da app atual.

## Sprint 6: Hardening, QA, Go-live e Cutover

### Tarefas
- [ ] S6-T01 Testes críticos (auth, RLS, CRUD dos 3 tipos, progresso, sync).
- [ ] S6-T02 Testes de regressão da secção séries.
- [ ] S6-T03 Observabilidade mínima (logs estruturados, erros por endpoint, métricas básicas).
- [ ] S6-T04 Segurança: rate-limit, validação input, revisão CSP.
- [ ] S6-T05 UAT com checklist formal + plano de rollback.
